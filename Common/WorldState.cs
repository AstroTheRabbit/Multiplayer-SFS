...
namespace MultiplayerSFS.Common
{
    public class WorldState
    {
        ...
        public WorldState(string path)
        {
            ...
            FolderPath persistent = folder.Extend("Persistent");
            ...
            if (!JsonWrapper.TryLoadJson(folder.ExtendToFile("WorldSettings.txt"), out WorldSettings settings))
                throw new Exception("'WorldSettings.txt' file cannot be found or does not exist.");

            if (!JsonWrapper.TryLoadJson(persistent.ExtendToFile("WorldState.txt"), out WorldSave.WorldState state))
                throw new Exception("'WorldState.txt' file cannot be found or does not exist.");

            if (!JsonWrapper.TryLoadJson(persistent.ExtendToFile("Rockets.txt"), out List<RocketSave> rocketSaves))
                throw new Exception("'Rockets.txt' file cannot be found or does not exist.");
            ...
            {
                AddRocket(new RocketState(save, idGenerator));
            }
        }
    ...
    public class RocketState : INetData
    {
        ...
        public RocketState(RocketSave save, Random idGenerator)
        {
